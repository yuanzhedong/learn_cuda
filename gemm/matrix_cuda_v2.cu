...
*********************************************************************
*/
__global__ void gpu_square_matrix_mult(int *d_M, int *d_N, int *d_result, int width) 
{
    ...
    //ph -> phrase
    for (int ph = 0; ph < ceil(width/(float)BLOCK_SIZE); ++ph) {

    	if ((row < width) && (ph*BLOCK_SIZE + tx) < width) {
	   Mds[ty][tx] = d_M[row*width + ph*BLOCK_SIZE + tx];
	} else {
	   Mds[ty][tx] = 0; // assign 0 is position is not within input dim
	}
...
int main(int argc, char const *argv[])
{
    ...
    int *h_a = NULL, *h_b, *h_c, *h_cc;
    cudaError_t err = cudaMallocHost((void **) &h_a, sizeof(int)*m*n);
